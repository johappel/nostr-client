...
## Ziel
Implementierung des Relay-Management-Systems für Verbindungen, Publishing und Subscriptions mit nostr-tools SimplePool.
...
### Schritt 1: Relay-Pool Setup

**Datei**: `framework/core/RelayManager.js`
...
export class RelayManager {
  ...
  async publish(event, relayUrls = null, timeout = 5000) {
    ...
    try {
      const pubs = this._pool.publish(targets, event);
      const results = [];

      // Wait for results with timeout
      const promises = pubs.map(pub => 
        this._waitForPublish(pub, timeout)
      );
      ...
      settled.forEach((result, index) => {
        ...
        if (result.status === 'fulfilled') {
          results.push({ relay, success: true, ...result.value });
   ...
   * @private
   */
  async _waitForPublish(pub, timeout) {
    return new Promise((resolve, reject) => {
...
// Test 6: Publish event (needs signed event)
const testEvent = {
  id: 'test-id-123',
  pubkey: 'test-pubkey',
...
// Note: This will fail with real relays (invalid signature)
// but tests the publish mechanism
try {
  const results = await manager.publish(testEvent, null, 2000);
  console.log('Publish results:', results);
} catch (error) {
  console.log('Publish test (expected to fail with test data):', error.message);
}

// Test 7: Get fastest relay
const fastest = await manager.getFastestRelay();
console.log('Fastest relay:', fastest);

// Test 8: Relay status
console.log('Relay status:', manager.getRelayStatus());

// Test 9: Events
manager.on('relay:event', (data) => {
  console.log('Event received via event bus:', data.event.id);
});

console.log('✓ All RelayManager tests completed!');
...
6. ➡️ Weiter mit `AGENT_EventManager.md`
