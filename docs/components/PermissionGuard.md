...
**Purpose**: Describe what users can accomplish with **PermissionGuard** in the Nostr UI kit.
...
## API (Props)
```ts
// Replace with precise props for PermissionGuard
...
## Code Snippet
```tsx
import { PermissionGuard } from "@/components/nostr-ui/PermissionGuard"
...
## Notes
Disable/hide controls if current auth/storage policy forbids a given action.
