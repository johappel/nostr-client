...
**Purpose**: Describe what users can accomplish with **RelayPicker** in the Nostr UI kit.
...
## API (Props)
```ts
// Replace with precise props for RelayPicker
export interface RelayPickerProps {
  className?: string
  // ...
}
```
...
## MVP
- Example page: `/examples/app/relaypicker/page.tsx`
...
## Code Snippet
```tsx
import { RelayPicker } from "@/components/nostr-ui/RelayPicker"
...
## Notes
UI to add/remove/toggle relays; integrates RelayHealthBadge per row.
