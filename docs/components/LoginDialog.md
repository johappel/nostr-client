...
**Purpose**: Describe what users can accomplish with **LoginDialog** in the Nostr UI kit.
...
## API (Props)
```ts
// Replace with precise props for LoginDialog
...
## Code Snippet
```tsx
import { LoginDialog } from "@/components/nostr-ui/LoginDialog"
...
## Notes
Presents provider choices with a modal flow; usually orchestrated by LoginSelector.
